...
export default function TrustIndicator({
  ...
  return (
    <div className={`bg-gradient-to-br from-white to-gray-50 rounded-2xl border-2 border-gray-100 ${sizeClasses.container}`}>
      ...
      {verificationStatus && (
        <div className="space-y-2 mb-4">
          ...
          <div className={`flex items-center justify-between ${sizeClasses.text}`}>
            ...
            {verificationStatus.ansut_certified && (
              <span className="text-green-600 font-semibold">Certifié</span>
            )}
          </div>
        </div>
      )}
      ...
      {!verificationStatus?.ansut_certified && profile && (
        <div className="mt-4 p-3 bg-amber-50 border-2 border-amber-200 rounded-xl">
          <div className="flex items-start space-x-2">
            <AlertCircle className="h-5 w-5 text-amber-600 flex-shrink-0 mt-0.5" />
            <div>
              <p className="text-xs text-gray-700 mb-2">
                Complétez votre certification pour augmenter votre score de confiance
              </p>
              <a
                href="/verification"
